...
/**
 * 基于继承关系的树操作工具
 * 若对性能有较高要求，可选择基于左右值编码的树结构工具包 {@link com.mcode.mybatisx.dal.util.tree.coding}
 * @param <S>
 * @param <R>
 */
public interface Tree<S extends TreeNode, R extends Tree<S, R>> {
    ...
    static <S extends TreeNode, R extends AbstractTree<S, R>> Tree<S, R> build(Supplier<R> supplier) {
        ...
        return r.setMapper((node, rSupplier) -> rSupplier.get().setAbstractTreeSupplier(supplier).setNode(node));
    }
